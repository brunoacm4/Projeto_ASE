...
esp_err_t st7735_init(const st7735_config_t *cfg) {
    ...
    ESP_LOGI(TAG, "Pinos: MOSI=%d MISO=%d CLK=%d CS=%d DC=%d RST=%d BL=%d",
             cfg->mosi_io_num, cfg->miso_io_num, cfg->sclk_io_num, cfg->cs_io_num,
    ...
    spi_bus_config_t buscfg = {
        .mosi_io_num = cfg->mosi_io_num, .miso_io_num = cfg->miso_io_num, .sclk_io_num = cfg->sclk_io_num,
